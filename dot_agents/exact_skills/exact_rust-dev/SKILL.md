...
- **Testing**: Unit tests inline in `#[cfg(test)] mod tests`. Async tests with `#[tokio::test]`. Feature-gated live tests (e.g., `#[cfg(feature = "test_online")]`). Prefer `assert_matches!` and `debug_assert_matches!` for pattern assertions once MSRV is 1.96 or newer. 90%+ coverage required.
...
- **Range APIs**: For public APIs, accept trait bounds such as `impl RangeBounds<T>` instead of concrete range types unless the concrete type is part of the contract. Once MSRV is 1.96 or newer, prefer `core::range`/`std::range` concrete range types when a stored range must be `Copy`.
- **Platform-specific code**: Prefer `cfg_select!` (stable since 1.95) over the `cfg-if` crate once MSRV allows. When upgrading to Rust 1.96 or newer, check WebAssembly builds for undefined symbols because they now link-error by default instead of being allowed imports.
- **Cargo.toml**: Edition 2024. Explicit `rust-version` for MSRV. Keep `rust-version`, `rust-toolchain.toml`, CI MSRV jobs, README, and AGENTS.md in sync when bumping MSRV. Features must be additive. Use `dep:` to hide optional dependencies from feature namespace. Pin with semver caret ranges. Configure `docs.rs` with `all-features` + `--cfg docsrs`.
...
- After finishing a non-trivial Rust task (feature, bugfix, review, or setup), briefly reflect on whether this skill helped or got in the way.
- Look for: mistakes you made, rules that were missing, wrong, or ambiguous, and friction a convention could have prevented.
- Only when a gap is general enough to help future Rust work, propose a concrete edit to this skill file in chat. Skip project-specific trivia and skip reflection entirely when nothing meaningful surfaced.
- Propose the exact change and wait for explicit approval. Do not edit this skill file or commit changes on your own.
