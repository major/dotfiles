...
- **Testing**: Unit tests inline in `#[cfg(test)] mod tests`. HTTP mocking with `mockito`. Async tests with `#[tokio::test]`. Feature-gated live tests (`#[cfg(feature = "test_online")]`). 90%+ coverage required.
- **Platform-specific code**: Prefer `cfg_select!` (stable since 1.95) over the `cfg-if` crate. Drop `cfg-if` from dependencies when MSRV allows.
...
- **Makefiles**: Standard targets: `check`, `fmt`, `fmt-fix`, `clippy`, `test`, `doc`, `coverage` (90%+ required), `audit`.
...
- **Anti-patterns**: Don't `.clone()` just to satisfy the borrow checker; restructure borrows or use scoped blocks instead. Never put `#[deny(warnings)]` in source code (breaks on compiler upgrades); use `-Dwarnings` in CI flags only.
