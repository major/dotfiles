...
## Writing issue, PR, and comment bodies

When creating or editing GitHub text bodies, write Markdown to a temp file under
`/tmp/opencode` and pass `--body-file <path>` instead of inline `--body` text.
This avoids shell quoting bugs with apostrophes, checkboxes, backticks, and
multiline Markdown.

- Prefer `gh issue create --title "..." --body-file /tmp/opencode/issue.md`.
- Prefer `gh issue edit 123 --body-file /tmp/opencode/issue.md`.
- Prefer `gh pr edit 123 --body-file /tmp/opencode/pr.md`.
- Use inline `--body` only for one-line text with no shell-sensitive characters.
